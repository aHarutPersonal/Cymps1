...
## 2024-05-25 - Optimize SQLAlchemy Pagination Counts
**Learning:** Using `select(func.count()).select_from(stmt.subquery())` for pagination counting is slower and less efficient as it creates an unnecessary subquery and retains potentially expensive `ORDER BY` clauses from the original query, degrading count performance.
**Action:** Replace this pattern with `stmt.with_only_columns(func.count(Model.id)).order_by(None)` to count records directly without a subquery and ignoring sort operations.
