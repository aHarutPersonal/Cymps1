...
## 2024-05-18 - SQLAlchemy Pagination Count Subquery Optimization
**Learning:** Using `select(func.count()).select_from(stmt.subquery())` is inefficient for total record counting before pagination, as it forces the database to generate a subquery.
**Action:** Replace it with `stmt.with_only_columns(func.count(Model.id)).order_by(None)` to count directly, taking care to drop the ORDER BY clause since ordering has no effect on counts.
