...
## 2024-05-24 - SQLAlchemy Pagination Count Subquery Bottleneck
**Learning:** Using `select(func.count()).select_from(stmt.subquery())` for pagination counts in SQLAlchemy generates an expensive, fully-evaluated subquery that drags down backend performance on large tables.
**Action:** Always use `stmt.with_only_columns(func.count(Model.id)).order_by(None)` instead to prevent inefficient subqueries and drop unnecessary sorting during counts.
