...
## 2024-06-18 - Optimize SQLAlchemy Pagination Counts
**Learning:** Using `select(func.count()).select_from(stmt.subquery())` for getting the total row count in paginated APIs creates an inefficient subquery wrapper that can be slower.
**Action:** Instead, modify the existing statement directly to fetch the count of the primary key using `stmt.with_only_columns(func.count(Model.id)).order_by(None)`. This avoids the wrapper subquery and allows the database engine to perform a more straightforward index or table scan.
