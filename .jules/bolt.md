...
## 2026-06-21 - Optimize SQLAlchemy Pagination Count Queries
**Learning:** Using `select(func.count()).select_from(stmt.subquery())` to count records for pagination is inefficient because it creates a subquery which the database has to execute before counting, adding unnecessary overhead.
**Action:** Use `stmt.with_only_columns(func.count(Model.id)).order_by(None)` instead. This efficiently modifies the existing select statement to only count the primary key and removes any sorting, which speeds up the count operation and avoids subquery overhead.
