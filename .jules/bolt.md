...
## 2024-06-06 - Optimized Pagination Counts for SQLAlchemy Queries
**Learning:** For endpoints calculating total records for pagination, `select(func.count()).select_from(stmt.subquery())` is inefficient as it instructs PostgreSQL to execute the full internal subquery before counting rows.
**Action:** Replace it with `stmt.with_only_columns(func.count(Model.id)).order_by(None)`. This constructs a straightforward `SELECT COUNT(Model.id) FROM Model WHERE ...` query directly, significantly improving performance on large datasets.
