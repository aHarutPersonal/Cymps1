...
## 2024-05-19 - SQLAlchemy Pagination Count Query Optimization
**Learning:** Found multiple instances of `select(func.count()).select_from(stmt.subquery())` in the codebase for pagination counting. This forces the database to evaluate a subquery before counting, which is inefficient.
**Action:** Replace this pattern with `stmt.with_only_columns(func.count(Model.id)).order_by(None)` to generate a direct, efficient count query and avoid unnecessary sorting overhead.
