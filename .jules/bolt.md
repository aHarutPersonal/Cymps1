...
## 2024-05-29 - Optimize SQLAlchemy Pagination Counts
**Learning:** The previous implementation used `select(func.count()).select_from(stmt.subquery())` for calculating total counts in paginated endpoints. Wrapping the original statement inside a subquery adds overhead on PostgreSQL compared to a standard count query.
**Action:** Replace this pattern with `stmt.with_only_columns(func.count(Model.id)).order_by(None)` which modifies the original query directly, preventing subquery creation and clearing any expensive sort operations during the count phase.
