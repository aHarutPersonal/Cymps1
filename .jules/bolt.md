...
## 2024-06-19 - Optimized SQLAlchemy Pagination Counts
**Learning:** Found a performance bottleneck where SQLAlchemy generated expensive subqueries to calculate pagination counts: `select(func.count()).select_from(stmt.subquery())`.
**Action:** Replaced it with `stmt.with_only_columns(func.count(Model.id)).order_by(None)` across multiple endpoints (`achievements.py`, `content_resources.py`, `idea_cards.py`, `notes.py`). This prevents the subquery overhead and drops unnecessary order_by clauses for counting.
