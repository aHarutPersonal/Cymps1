...
## 2025-02-14 - Optimize Pagination Count Queries
**Learning:** Found several performance bottlenecks where `select(func.count()).select_from(stmt.subquery())` was used to calculate the total count for pagination. This pattern is inefficient as it creates an unnecessary and slow subquery in PostgreSQL.
**Action:** Replace this pattern with `stmt.with_only_columns(func.count(Model.id)).order_by(None)` to calculate the count efficiently without subqueries and avoiding order-by overhead.
