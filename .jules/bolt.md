...
## 2024-05-25 - Avoid subqueries for pagination counting
**Learning:** Using `select(func.count()).select_from(stmt.subquery())` creates an inefficient subquery wrapper that can slow down database performance, especially when there's an outer join or complex filtering.
**Action:** Use `stmt.with_only_columns(func.count(Model.id)).order_by(None)` instead to compute counts directly without creating a subquery. The `order_by(None)` is crucial to strip any sorting that isn't needed for counting.
