...
## 2024-05-25 - Optimize SQLAlchemy Pagination Count Queries
**Learning:** Using `select(func.count()).select_from(stmt.subquery())` to calculate the total number of items for pagination creates inefficient nested SQL queries, which causes performance bottlenecks.
**Action:** Replace it with `stmt.with_only_columns(func.count(Model.id)).order_by(None)` to efficiently modify the existing query statement to compute the count without the overhead of generating and executing a subquery.
