...
## 2024-05-24 - Optimize Pagination Count Queries
**Learning:** Using `select(func.count()).select_from(stmt.subquery())` for pagination counting in SQLAlchemy generates inefficient nested subqueries `SELECT count(*) FROM (SELECT ... FROM table)`.
**Action:** Replace it with `stmt.with_only_columns(func.count(Model.id)).order_by(None)`. This produces a flat, direct count `SELECT count(table.id) FROM table`, bypassing the need to load all columns inside a subquery. It's also important to strip the `order_by` clause because ordering is meaningless for a scalar count result and adds overhead.
