...
## 2024-06-02 - Backend SQLAlchemy Pagination Count Optimization
**Learning:** Using `select(func.count()).select_from(stmt.subquery())` for pagination counting creates potentially inefficient subqueries and retains ordering clauses (`ORDER BY`), slowing down database queries.
**Action:** Always use `stmt.with_only_columns(func.count(Model.id)).order_by(None)` instead to execute a direct count and strip out expensive ordering operations.
