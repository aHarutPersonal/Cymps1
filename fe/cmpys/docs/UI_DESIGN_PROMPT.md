...
- Progress messaging with fluid shimmer effect

**No Plan State**:
- Empty state illustration (Deepstash minimalist style)
- "Generate Plan" CTA button with glowing gradient border

**Plan Content**:
- Week selector/navigator (horizontal swipeable pills with snap physics)
- Date range display
- Weekly Goal Card:
  - Deepstash-style glass card with subtle category-colored glow.
  - Large, animated circular progress ring (spring physics when completing tasks).
- Task Cards for each plan item:
  - Checkbox with haptic feedback and micro-animation on completion.
  - Title and description (highly readable premium typography).
  - Category color badge.
  - Lesson/link indicator seamlessly integrated.
  - On tap, the card smoothly expands using a Hero transition or immersive bottom sheet.
- Empty Week state for weeks without tasks

**Regenerate Dialog**:
- Confirmation modal with frosted glass effect
- Warning about losing progress
- Cancel/Confirm buttons

**Task Detail Screen** (when tapping a task):
- Expands into a full-screen vertical Idea Card.
- Learning resources and Action items presented as scannable, bite-sized blocks.
...
### 8. Notes & Personal Stash Screen

**Header** with title "Stash" and add button

**Search Bar** for filtering notes and saved ideas

**Notes & Idea List**:
- Note/Idea cards with:
  - Title or Idea Snippet
  - Source/Idol Attribution (for stashed ideas)
  - Date saved
  - Attachment badges (image/doc/audio indicators)
...
### 12. Agentic Workflow (Advanced Setup & Guided Learning)

**Agentic Intake Screen**
- Form requesting: Age, Financial Status, and Key Interests
- "Generate Recommendations" CTA that initiates Phase 1 of the workflow
- Loading overlay during submission

**Idol Pick Screen**
- Grid/list of 3 highly tailored idol suggestions generated by the AI
- Idol cards showing name, era, and a brief "Why they fit" relevance summary
- "Select Mentor" action on each card

**Interview Screen**
- Chat-style UI where the user converses with the selected AI mentor
- Phase progress indicator (e.g., Turn counter)
- Lock text input while AI is streaming its response
- Auto-transitions to the next phase on completion

**Results Screen (Blueprint & Comparison)**
- Full-screen rich text display rendering Markdown (`flutter_markdown`)
- **Comparison Section** ("The Brutal Truth"): Highlights differences between user and idol
- **Blueprint Section** ("Your Quarterly Blueprint"): Shows Q1-Q4 actionable goals
- Streaming animation during generation
- **"Start Guided Learning"** CTA button

**Guided Learning Screen (Phase 6)**
- Header showing the selected learning topic
- **Top Section (Recommended Materials):** Horizontal scrolling list of article and video cards (fetched via Gemini Grounding/Search)
- **Main Section (Socratic Chat):** Dedicated chat interface for learning. Uses the LearnLM model to help users learn via the Socratic method rather than just receiving answers.
- Text input field and send button (disabled while streaming responses)

---

### 13. Daily Insights Feed (Microlearning)

**Header**:
- Title "Daily Feed" or "Your Insights"
- Date and Streak indicator

**Idea Cards (Swipeable Vertical Feed)**:
- Full-screen or large vertical glass/pastel cards on a dark background.
- Scannable, bite-sized knowledge snippets (< 200 words).
- Idol attribution / Source of the idea.
- **Audio Playback (TTS)**: Floating play button to read the insight aloud.
- **Stash Button**: Bookmark icon to instantly save the card to "Personal Stash" (Notes).
- **Share/Like Actions**.
- Smooth swipe animations (TikTok/Reels style scrolling for reading).
...
| Idea Cards | Large vertical cards with vibrant pastels/gradients, premium readable typography, and floating action buttons (Audio/Stash) |

---

## Design Aesthetic Notes

- **Theme**: Dark mode primary, enhanced with deep glows and vibrant gradients.
- **Colors**: Category-specific (Career=Blue, Education=Purple, Skills=Green, Personal=Orange, Creativity=Pink). Use vibrant pastels for Idea Cards to contrast against the dark background.
- **Typography**: Modern, premium sans-serif (e.g., Inter, Outfit) with high readability and generous line height for microlearning. Clear hierarchy (H1, H2, H3, H4, Body, Caption).
- **Spacing**: Consistent padding system (8, 12, 16, 20, 24, 32px), very breathable layouts.
- **Animations**: Subtle micro-interactions, swipeable vertical feeds (TikTok style), fluid card transitions, typing indicators, progress animations.
...
├── Agentic Workflow (Alternative Setup)
│   ├── Agentic Intake
│   ├── Idol Pick
│   ├── Interview
│   ├── Results (Blueprint/Comparison)
│   └── Guided Learning
```
