...
enum ComparisonScoresSyncResult { ready, unavailable, timedOut, failed }
...
final cmpysComparisonScoresSyncProvider =
    FutureProvider.autoDispose<ComparisonScoresSyncResult>((ref) async {
      ...
      for (var attempt = 0; attempt < 12; attempt++) {
        ...
        if (session.comparisonScoresStatus == 'failed') {
          return ComparisonScoresSyncResult.failed;
        }
        if (session.comparisonScoresStatus == 'ready') {
          // A ready lifecycle without readable v2 dimensions is terminal for this
          // payload; do not turn a malformed response into an endless spinner.
          return ComparisonScoresSyncResult.failed;
        }
        if (state.comparisonMd?.trim().isNotEmpty != true) {
          return ComparisonScoresSyncResult.unavailable;
