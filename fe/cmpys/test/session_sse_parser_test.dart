...
void main() {
  ...
  test('parseSseEvents skips a malformed data line without dropping valid ones',
      () async {
    final chunks = <List<int>>[
  ...
  test('parseSseEvents handles an em-dash split across the chunk boundary',
      () async {
    // "—" is 3 UTF-8 bytes (e2 80 94); split it down the middle.
    final bytes = utf8.encode('data: {"type":"chunk","content":"a—b"}\n\n');
    final emDashStart = utf8.encode('data: {"type":"chunk","content":"a').length;
    final chunks = <List<int>>[
