"""Harden curriculum insert audit shapes and repair stamped artifact progress.
            ...
            sa.text(
                ...
                BEGIN
                    IF EXISTS (
                        ...
                        RAISE EXCEPTION
                            'cannot upgrade h8: {table_name} contains ambiguous lifecycle audit data';
                    END IF;
                END;
                $$;
                """
...
def _create_audit_shape_constraints() -> None:
...
def _affected_artifact_predicate() -> str:
    return f"(({MALFORMED_STEPS}) OR ({PARTIAL_ARTIFACT}))"
...
def _repair_stamped_artifact_progress() -> None:
    ...
    op.execute(
        sa.text(
            ...
            SET status = CASE
                    WHEN ({MALFORMED_STEPS})
                      OR ({INVALID_PROGRESSIVE_READY_STEP_IDS})
                ...
                progress_percent = CASE
                    WHEN ({MALFORMED_STEPS})
                      OR ({INVALID_PROGRESSIVE_READY_STEP_IDS}) THEN 0
                    ELSE LEAST(
                        99,
                        ROUND(
                            100.0 * (
...
def upgrade() -> None:
    # This must remain the first operation. PostgreSQL executes the whole
    # revision transactionally, so an ambiguous audit row leaves no partial DDL.
    _preflight_lifecycle_shapes()
    _create_audit_shape_constraints()
    _repair_stamped_artifact_progress()
    _replace_plan_assignment_guard(protect_terminal_history=True)


def downgrade() -> None:
    # Data repaired above remains safely fail-closed; fabricating old completion
    # timestamps during downgrade would be lossy and misleading.
    _replace_plan_assignment_guard(protect_terminal_history=False)
    for table_name, constraint_name, _expression in reversed(AUDIT_SHAPE_CONSTRAINTS):
