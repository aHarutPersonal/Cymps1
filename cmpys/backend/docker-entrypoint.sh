...
case "$ROLE" in
  ...
  worker)  exec celery -A app.core.celery.celery_app worker --loglevel=info --concurrency="${CELERY_CONCURRENCY:-4}" -Q default,high_priority,low_priority,catalog,catalog_control ;;
