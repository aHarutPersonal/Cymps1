...
case "$ROLE" in
  ...
  worker)  exec celery -A app.core.celery.celery_app worker --loglevel=info --pool="${CELERY_DEFAULT_POOL:-solo}" --concurrency="${CELERY_DEFAULT_CONCURRENCY:-1}" -Q default ;;
  worker-high) exec celery -A app.core.celery.celery_app worker --loglevel=info --pool="${CELERY_HIGH_POOL:-solo}" --concurrency="${CELERY_HIGH_CONCURRENCY:-1}" -Q high_priority ;;
