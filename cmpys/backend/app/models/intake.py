...
class IntakeSession(Base, UUIDMixin, TimestampUpdateMixin):
    ...
    # Durable lifecycle for the structured comparison scorer.  The JSON
    # remains null until a valid v2 result exists; these fields let clients and
    # workers distinguish queued/running/retryable/terminal states instead of
    # treating every absence as an endless spinner.
    comparison_scores_status: Mapped[str] = mapped_column(
        String(24),
        nullable=False,
        default="not_started",
    )
    comparison_scores_attempts: Mapped[int] = mapped_column(
        Integer,
        nullable=False,
        default=0,
    )
    comparison_scores_error: Mapped[str | None] = mapped_column(
        Text,
        nullable=True,
        default=None,
    )
    comparison_scores_last_attempt_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
        default=None,
    )
    comparison_scores_next_retry_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
        default=None,
    )
