...
cleanup() {
    if [ "$USE_LOCAL_BACKEND" = true ]; then
        ...
        if [ ! -z "$CURRICULUM_CONTROL_PID" ]; then
            kill -TERM "$CURRICULUM_CONTROL_PID" 2>/dev/null || true
        fi
        if [ ! -z "$CURRICULUM_WORKER_PID" ]; then
            kill -TERM "$CURRICULUM_WORKER_PID" 2>/dev/null || true
        fi
...
echo "Starting Curriculum Worker..."
CURRICULUM_WORKER_CONCURRENCY="${CURRICULUM_WORKER_CONCURRENCY:-2}"
celery -A app.core.celery worker \
    -n curriculum@%h \
    -Q curriculum \
    --concurrency="$CURRICULUM_WORKER_CONCURRENCY" \
    --loglevel=info > celery-curriculum.log 2>&1 &
CURRICULUM_WORKER_PID=$!
echo "✅ Curriculum worker running in background"
echo "   (PID: $CURRICULUM_WORKER_PID, concurrency: $CURRICULUM_WORKER_CONCURRENCY)"

echo "Starting Curriculum Control Worker..."
celery -A app.core.celery worker -n curriculum-control@%h -Q curriculum_control --concurrency=1 --loglevel=info > celery-curriculum-control.log 2>&1 &
CURRICULUM_CONTROL_PID=$!
echo "✅ Curriculum control worker running in background (PID: $CURRICULUM_CONTROL_PID)"
...
for _ in {1..20}; do
    if ! kill -0 "$CELERY_PID" 2>/dev/null \
        ...
        || ! kill -0 "$CATALOG_CONTROL_PID" 2>/dev/null \
        || ! kill -0 "$CURRICULUM_WORKER_PID" 2>/dev/null \
        || ! kill -0 "$CURRICULUM_CONTROL_PID" 2>/dev/null; then
        break
    fi
    PONG_COUNT=$(celery -A app.core.celery inspect ping --timeout=1 2>/dev/null | grep -c "pong" || true)
    if [ "$PONG_COUNT" -ge 5 ]; then
        WORKER_READY=true
        break
    fi
    sleep 0.5
